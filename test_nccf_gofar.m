...
% filename = 'NCCF_OBS01_03_CompZ_fcl0_25_fch0_5_NobitNorm_TSA_VDAresults';
% print('-dpdf','-r300',filename)

%%
clear cabs cdyn csta csv xc_avg xc 
[cabs,cdyn,csta,csv,xc_avg,xc] = nccf(data1,data2,segle,fcl,fch,...
    1/fs,norm1b,funcle);
% 
% figure; plot(data1)
% figure; plot(data2)

%% Check frequency content of marmara time series at low frequencies
% Welch averaged PSD

clear
addpath('/home/jbtary/Documents/MATLAB/SAC')

ii = 1118; % Index of first data file
dfac = 20; % Decimation factor
fs = 100; % Sampling frequency
nfiles = 72; % # of files to concatenate
fs = fs/dfac; % New sampling frequency

dir1 = '/media/jbtary/DeLaFeuille/BackUP/Marmara/Data/';
dir2 = '/media/jbtary/DeLaFeuille/BackUP/Marmara/Data/';
obs1 = 'K04'; co1 = 'Z'; cn1 = 'z'; % obs2 = 'G10'; co2 = 'Z'; cn2 = '2';
obs2 = 'K04'; co2 = 'Z'; cn2 = 'z';
[info1,info2] = format_name_nccf(dir1,dir2,obs1,obs2,co1,co2);

data1 = [];
for jj = ii:ii+nfiles-1
    datatmp = rdSac([dir1 obs1 '/' obs1 co1 '/' info1(jj).name(1:23) cn1 '.sac']);
%     datatmp = rdSac([dir2 info2(jj).name]);
    % Concatenation before decimation to avoid steps in data streams
    data1 = [data1;datatmp];
    clear datatmp
end
clear jj

data1 = decimate(data1,dfac);
data1 = data1 - mean(data1);

[pwa,freq] = pwelch(data1,3600*fs,0.9*3600*fs,2048,fs);

plot(freq,10*log10(pwa))
